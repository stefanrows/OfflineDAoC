...
if ($PSCmdlet.ParameterSetName -eq 'Restore') {
    ...
    foreach ($pair in @(@($game, $record.OutputGameSha256, $record.BaselineGameSha256),
        ...
        if ($current -ne $pair[1] -and $current -ne $pair[2]) {
            throw "Installed file changed since the probe: $($pair[0]) ($current). Restore refused."
        }
    }
    ...
    Write-Host "Custom8 probe restored from $backup; backup retained."
    return
}
...
Write-Host "Staged probe game.dll:     $($manifest.outputSha256)"
...
$backup = "$root-backups\companion-custom8-probe-$stamp"
...
try {
    ...
    Write-Host "Custom8 probe installed. Backup: $backup"
}
catch {
